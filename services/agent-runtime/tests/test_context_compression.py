...
def test_full_strategy_keeps_recent_within_budget():
...
def test_summary_strategy_bounded_and_deterministic():
    turns = _turns(20, size=200)
...
def test_summary_strategy_accepts_injected_summarizer():
...
def test_empty_turns_yield_empty_history():
...
def test_in_memory_long_term_store_put_retrieve():
...
def test_build_request_model_history_is_bounded():
...
def test_request_context_injects_model_history_when_enabled(monkeypatch):
    from allcallall_agent_runtime import helpers
    from allcallall_agent_runtime.models import ConversationMessage, MeetingBriefRequest

    monkeypatch.setattr(helpers.app_config, "enable_context_compression", True)
    request = MeetingBriefRequest(
        organization_id=1, user_id=1, conversation_id=1, workflow_run_id=1, goal="g",
    ...
    assert estimate_tokens(out.model_history) <= helpers.app_config.model_history_max_tokens


def test_request_context_omits_model_history_when_disabled(monkeypatch):
    from allcallall_agent_runtime import helpers
    from allcallall_agent_runtime.models import ConversationMessage, MeetingBriefRequest

    monkeypatch.setattr(helpers.app_config, "enable_context_compression", False)
    request = MeetingBriefRequest(
        organization_id=1, user_id=1, conversation_id=1, workflow_run_id=1, goal="g",
